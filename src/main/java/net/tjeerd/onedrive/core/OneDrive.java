...
public class OneDrive implements OneDriveAPI {
	...
	public OneDrive(Principal principal) {
		...
		defaultClientConfig.getFeatures().put(
	...
	public boolean initAccessTokenByRefreshTokenAndClientId() {
		if (hasPrincipalRefreshTokenAndClientId()) {
			this.principal.getoAuth20Token().setAccess_token(
					oneDriveCore.getAccessTokenByRefreshTokenAndClientId()
							.getAccess_token());
			return true;
		} else {
			return false;
	...
	private boolean hasPrincipalClientSecretAndClientIdAndAuthorizationCode() {
		return (StringUtils.isNotEmpty(principal.getClientId())
				&& StringUtils.isNotEmpty(principal.getClientSecret()) && StringUtils
					.isNotEmpty(principal.getAuthorizationCode()));
	}
	...
	private boolean hasPrincipalRefreshTokenAndClientId() {
		return (StringUtils.isNotEmpty(principal.getoAuth20Token()
				.getRefresh_token()) && StringUtils.isNotEmpty(principal
				.getClientId()));
	}
	...
	public User getUser() throws Exception {
		return (User) oneDriveCore.doGetAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, API_PATH_ME, new User());
	}
	...
	public Quota getQuota() throws Exception {
		return (Quota) oneDriveCore.doGetAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, API_PATH_ME_SKYDRIVE_QUOTA,
				new Quota());
	}
	 ...
	 * {@inheritDoc}
	 */
	public Folder getMyFilesList(FriendlyNamesEnum friendlyNamesEnum)
			throws Exception {
		...
		return (Folder) oneDriveCore.doGetAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, apiPath, new Folder());
	}
	...
	public Folder getFileList(String folderId) throws Exception {
		return (Folder) oneDriveCore.doGetAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, folderId + "/" + API_PATH_FILES,
				new Folder());
	}
	 ...
	 * {@inheritDoc}
	 */
	public void downloadFile(net.tjeerd.onedrive.json.folder.File oneDriveFile,
			String destinationFilePath) {
		ClientResponse clientResponse = oneDriveCore.doGetAPI(
				new MultivaluedMapImpl(), MediaType.APPLICATION_OCTET_STREAM,
				oneDriveFile.getId() + "/" + API_PATH_CONTENT);
		java.io.File destinationFile = new java.io.File(destinationFilePath);

		if (clientResponse.getStatus() == ClientResponse.Status.FOUND
				.getStatusCode()) {
			/*
			 ...
			 */
			try {
				clientResponse = oneDriveCore.doGetAPI(clientResponse
						.getLocation().toString(),
						MediaType.APPLICATION_OCTET_STREAM);
				java.io.File resultFile = clientResponse
						.getEntity(java.io.File.class);
				org.apache.commons.io.FileUtils.moveFile(resultFile,
						destinationFile);
				logger.info("OneDriveAPI file '" + oneDriveFile.getName()
						+ "' saved to '" + destinationFilePath + "' ("
						+ destinationFile.length() + " bytes)");
			} catch (Exception exception) {
				logger.error("Could not download from redirect location "
						+ clientResponse.getLocation());
				exception.printStackTrace();
			}
		} else if (clientResponse.getStatus() == ClientResponse.Status.OK
				.getStatusCode()) {
			/*
			...
			try {
				...
				logger.error("Cannot download or write file with identifier '"
						+ oneDriveFile.getId() + "' to destination '"
						+ destinationFilePath + "'");
				e.printStackTrace();
			}
		}
	 ...
	 * {@inheritDoc}
	 */
	public net.tjeerd.onedrive.json.folder.File getFile(String fileId)
			throws Exception {
		return (net.tjeerd.onedrive.json.folder.File) oneDriveCore.doGetAPI(
				new MultivaluedMapImpl(), MediaType.APPLICATION_JSON, fileId,
				new net.tjeerd.onedrive.json.folder.File());
	}

	/**
	 * {@inheritDoc}
	 */
	public net.tjeerd.onedrive.json.folder.Folder getFolder(String folderId)
			throws Exception {
		return (net.tjeerd.onedrive.json.folder.Folder) oneDriveCore.doGetAPI(
				new MultivaluedMapImpl(), MediaType.APPLICATION_JSON, folderId,
				new net.tjeerd.onedrive.json.folder.Folder());
	}

	/**
	 * {@inheritDoc}
	 */
	public void deleteFile(net.tjeerd.onedrive.json.folder.File oneDriveFile)
			throws RestException {
		oneDriveCore.doDeleteAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, oneDriveFile.getId());
	}

	/**
	 * {@inheritDoc}
	 */
	public net.tjeerd.onedrive.json.folder.File updateFile(
			net.tjeerd.onedrive.json.folder.File oneDriveFile)
			throws RestException, IOException {
		return (net.tjeerd.onedrive.json.folder.File) oneDriveCore.doPutAPI(
				new MultivaluedMapImpl(), MediaType.APPLICATION_JSON,
				oneDriveFile.getId(), oneDriveFile);
	}

	/**
	 * {@inheritDoc}
	 */
	public net.tjeerd.onedrive.json.folder.Folder updateFolder(
			net.tjeerd.onedrive.json.folder.Folder oneDriveFolder)
			throws RestException, IOException {
		return (Folder) oneDriveCore.doPutAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, oneDriveFolder.getId(),
				oneDriveFolder);
	}
	 ...
	 * {@inheritDoc}
	 */
	public net.tjeerd.onedrive.json.folder.Folder createFolder(String name,
			String description, String locationFolderId) throws RestException,
			IOException {
		String apiPath;
		...
		return (Folder) oneDriveCore.doPostAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, apiPath, oneDriveFolder);
	}
	...
	public void deleteFolder(String folderId) throws RestException {
		oneDriveCore.doDeleteAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, folderId);
	}

	/**
	 * {@inheritDoc}
	 */
	public net.tjeerd.onedrive.json.folder.File uploadFile(File file,
			String folderId) {
		...
		queryParams.add(OneDriveCore.API_PARAM_ACCESS_TOKEN, principal
				.getoAuth20Token().getAccess_token());
		WebResource webResource = client.resource(OneDriveEnum.API_URL
				.toString() + apiPath);

		try {
			ClientResponse clientResponse = webResource
					.queryParams(queryParams).type(MediaType.APPLICATION_JSON)
					.put(ClientResponse.class, fileToByteArray(file));
			oneDriveFile = objectMapper.readValue(
					clientResponse.getEntity(String.class).toString(),
					net.tjeerd.onedrive.json.folder.File.class);
		} catch (Exception e) {
			logger.error("Cannot upload file '" + file.getAbsolutePath()
					+ "' to OneDriveAPI");
			e.printStackTrace();
		}
	...
	public net.tjeerd.onedrive.json.folder.File uploadLargeFile(File file,
		...
		try {
			...
			queryParams.add(OneDriveCore.API_PARAM_ACCESS_TOKEN, principal
					.getoAuth20Token().getAccess_token());
			webResource = client.resource(OneDriveEnum.API_URL_ONEDRIVE
					.toString() + apiPath);
			...
			clientResponse = webResource
					...
					.post(ClientResponse.class,
							objectMapper
									.writeValueAsString(LARGE_FILE_POST_REQUEST_BODY));
			...
			uploadSession = objectMapper.readValue(
					clientResponse.getEntity(String.class).toString(),
					UploadSession.class);

			// Upload file fragments
			webResource = client.resource(uploadSession.getUploadUrl());
			while (0 < (readBytes = fileInputStream.read(fileChunkBuffer, 0,
					LARGE_FILE_FRAGMENT_MAX_SIZE))) {
				
				if(fileChunkBuffer.length != LARGE_FILE_FRAGMENT_MAX_SIZE) {
				...
				clientResponse = webResource
						...
						.header("Content-Range",
								String.format("bytes %d-%d/%d",
										uploadBytesCount, uploadBytesCount
												+ readBytes - 1, file.length()))
			...
			oneDriveFile = new net.tjeerd.onedrive.json.folder.File();
		} catch (Exception e) {
			logger.error("Cannot upload file '" + file.getAbsolutePath()
					+ "' to OneDriveAPI");
			e.printStackTrace();
	 ...
	 * {@inheritDoc}
	 */
	public SharedLink getSharedLink(String fileId, boolean isEditable)
			throws Exception {
		String apiPath;
		...
		return (SharedLink) oneDriveCore.doGetAPI(new MultivaluedMapImpl(),
				MediaType.APPLICATION_JSON, apiPath, new SharedLink());
	}
}
